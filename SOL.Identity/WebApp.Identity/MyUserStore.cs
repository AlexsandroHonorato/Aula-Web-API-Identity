...
namespace WebApp.Identity {
    public class MyUserStore : IUserStore<MyUser>, IUserPasswordStore<MyUser> {
        ...
        public async Task<IdentityResult> CreateAsync(MyUser user, CancellationToken cancellationToken) {
            using (var connection = GetOpenConnection()) {
                await connection.ExecuteAsync(
                    ...
                    "[NormalizedUserName], " +
                    "[PasswordHash]) " +
                    "values (@id, @name, @normalizedUserName, @passwordHash)",
                    new {
                        id = user.Id,
                        name = user.UserName,
        ...
        public static DbConnection GetOpenConnection() {
            var connection = new SqlConnection(@"Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=IdentityCurso;Data Source=DESKTOP-6H183G7\SQLEXPRESS");
            connection.Open();
